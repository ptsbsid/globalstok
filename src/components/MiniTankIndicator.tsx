  ...
  return (
    <div className="space-y-1">
      <div className="flex justify-between items-center mb-1">
        ...
        <span className={`text-xs md:text-sm font-bold ${color.replace('bg-', 'text-')}`}>
          {percentage}%
        </span>
      </div>
      <div className="w-full h-14 md:h-16 bg-gray-700 rounded-md relative overflow-hidden border border-gray-600">
        {/* Tank background */}
        <div 
        ...
        <div 
          className="absolute inset-0 flex items-center justify-center text-white text-[11px] md:text-xs font-semibold z-10 bg-black bg-opacity-30 rounded"
        >
          Terisi: {formatStockAmount(current)}
        </div>
      </div>
      <div className="flex justify-end text-[11px] md:text-xs text-gray-400 mt-1">
